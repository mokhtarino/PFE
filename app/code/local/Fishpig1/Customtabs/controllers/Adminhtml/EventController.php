 ...
 */
class Fishpig_Customtabs_Adminhtml_EventController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        $this->loadLayout();
        $this->_setActiveMenu('customtabs/events');

        $this->_addContent(
            $this->getLayout()->createBlock('customtabs/adminhtml_event')
        );
    ...
    public function editAction()
    {
        if ( $eventId = $this->getRequest()->getParam('event_id') ) {
            Mage::register('current_event', Mage::getModel('customtabs/event')->load($eventId));
        }
        ...
        $this->_addContent(
            $this->getLayout()->createBlock('customtabs/adminhtml_event_edit')
        );
    ...
    public function saveAction()
    {
        ...
        $eventModel = Mage::getModel('customtabs/event')->load($eventId);
        ...
        if ( $eventIds = $this->getRequest()->getParam('event_ids') ) {
            try{
                foreach ($eventIds as $eventId) {
                    $model = Mage::getModel('customtabs/event')->load($eventId);
                    $model->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    {
        $fileName    = 'events.csv';
        $grid        = $this->getLayout()->createBlock('customtabs/adminhtml_event_grid');
